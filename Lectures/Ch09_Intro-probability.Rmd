...
- For the conitnuous example, the event could be a value between 0.4 and 0.5.
...
- The probability that the CS rate is larger than 40% is written as $P(CS > 0.4)$
We can also write it as $P1 - P(CS \leq 0.4)$ by applying the complement rule.
...
in a specified range, e.g., $P(X < x)$, $P(X >= x)$, or $P(x_1 < X < x_2)$
