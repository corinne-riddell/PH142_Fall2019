---
title: 'Lab 4: Probability Practice Solutions'
...
This is equal to `sum(one_hundred_flips_2)`. Very likely not the same, although could be for someone!
...
Event that is relatively common: Prob(HPV) among non-vaccinated adults is 43% in men, 39% in women.
Event that is relatively rare: Prob of birth defect (2-4%)
...
*Method 1:*  
We can use the **complement rule**, which is the idea that for any event A,

**P(A does not occur) = 1 - P(A)**. Can also be written as:
$P(A^c) = 1 - P(A)$, or $P(A') = 1 - P(A)$

Then,   
P(Blood type $\neq$ A+) = 1 - P(Blood type = A+)
= 1 - 0.27  
= 0.73  

*Method 2:*   
P(Blood type $\neq$ A+) = P(Blood type = O+, O-, A-, B+, B-, AB+, AB-)    
= 0.39 + 0.01 + 0.005 + 0.25 + 0.004 + 0.07 + 0.001    
= 0.73  
...
Then:
P(Blood type = O+ or blood type = O-)   
= P(Blood type = O+) + P(Blood type = O-)  
= 0.39 + 0.01  
= 0.40 
...
Fill in the blank: The probability of any **individual** value under a continuous distribution is always _zero_.
...
Write your answer here. Solution: 0.4

**Question**: What does $P(X \geq 0.4)$ equal? Make a sketch on paper of the area being calculated.

Write your answer here. Solution: 0.6
...
#this takes the area below 0.5, and then subtract the area below 0.3 to leave the area in the middle
```

Solution: 0.2, shade the area between 0.3 and 0.5 on the plot.
...
punif(0.3, 0, 1) + punif(0.5, 0, 1, lower.tail = F)
```

Solution: 0.8, shade the area below 0.3, and separately the area above 0.5 
